...
(function($) {
    ...
    function openCropperModal(attachment) {
        ...
        img.off('load').on('load', function() {
            ...
            try {
                // Get per-axis crop dimensions; fall back to legacy cropping_size for existing installs
                const cropFallback = medbeafgalleryAdmin.cropping_size || 800;
                const cropWidth  = medbeafgalleryAdmin.cropping_width  || cropFallback;
                const cropHeight = medbeafgalleryAdmin.cropping_height || cropFallback;
                debug('Using crop size: ' + cropWidth + 'x' + cropHeight + 'px');

                // Initialize Cropper with aspect ratio derived from width/height
    ...
    function applyCrop() {
        ...
        try {
            // Get per-axis crop dimensions; fall back to legacy cropping_size for existing installs
            const cropFallback = medbeafgalleryAdmin && medbeafgalleryAdmin.cropping_size ? medbeafgalleryAdmin.cropping_size : 800;
            const cropWidth  = medbeafgalleryAdmin && medbeafgalleryAdmin.cropping_width  ? medbeafgalleryAdmin.cropping_width  : cropFallback;
            const cropHeight = medbeafgalleryAdmin && medbeafgalleryAdmin.cropping_height ? medbeafgalleryAdmin.cropping_height : cropFallback;
            debug('Creating cropped canvas with size: ' + cropWidth + 'x' + cropHeight);
