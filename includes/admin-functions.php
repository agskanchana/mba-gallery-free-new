...
function medbeafgallery_admin_enqueue_scripts($hook) {
    ...
    // Derive per-axis crop dimensions with fallback to legacy cropping_size
    $crop_fallback = absint( $settings['cropping_size'] ?? 800 ) ?: 800;
    $crop_width  = ! empty( $settings['cropping_width'] )  ? absint( $settings['cropping_width'] )  : $crop_fallback;
    $crop_height = ! empty( $settings['cropping_height'] ) ? absint( $settings['cropping_height'] ) : $crop_fallback;
    ...
    $admin_data = array(
        ...
        'cropping_size' => $crop_fallback,
        'cropping_width' => $crop_width,
        'cropping_height' => $crop_height,
